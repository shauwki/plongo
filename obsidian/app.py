...
@app.route("/")
def index():
    """Serveert de front-end web applicatie."""
    return render_template('index.html')

@app.route("/notes", methods=['GET', 'POST'])
@require_api_key
def handle_notes_collection():
    """Handles listing all notes (GET) and creating a new note (POST)."""
    if request.method == 'GET':
        notes_subdir = NOTES_DIR.joinpath('notes')
        if not notes_subdir.is_dir():
            return jsonify([])
...
def handle_single_note(note_path):
    """Handles reading (GET), updating (PUT), and deleting (DELETE) a single note."""
